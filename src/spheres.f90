module spheres
    ...
    subroutine read_spheres(cluster_file_name,a_eff,max_dist)
        ...
        !write(stderr,*) matmul(rm,inverse(rm))
        ...
        do i=1,nspheres
            ...
            !write(stderr, *) n, pos(i,1), pos(i,2), pos(i,3), radii(i)
            !write(stderr, *) pos(i,1) + pos(i,2)
            ! orient grain so that the A_1 vector is in the direction of propagation
            pos(i,:) = matmul(rm,pos(i,:))
            !write(stderr, *) pos(i,1), pos(i,2), pos(i,3)
        ...
        max_dist = 0.0
       ! write(stderr, *) "nspheres", nspheres, "a_eff_temp", a_eff_temp, "conv", conv
        do i=1,nspheres
            pos(i,1) = pos(i,1)*conv
            pos(i,2) = pos(i,2)*conv 
            pos(i,3) = pos(i,3)*conv
            radii(i) = radii(i)*conv   
            !write(stderr, *) "pos1", pos(i,1), "pos2", pos(i,2), "pos3",pos(i,3), "r", radii(i)
