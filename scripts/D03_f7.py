...
def set_axis(emin,emax,name,AXIS,lines=[],comp_data=None,comp_color='r'):
	...
	AXIS.plot(data['E'],data['sig_abs'],ls='--',color='k')
	AXIS.plot(data['E'],data['sig_scat'],ls='-',color='k')
	AXIS.plot(data['E'],data['sig_scat']+data['sig_abs'], lw=2,color='k')
	
	if (comp_data != None):
		AXIS.plot(comp_data['E'],comp_data['sig_abs'],ls='--',color=comp_color)
		AXIS.plot(comp_data['E'],comp_data['sig_scat'],ls='-',color=comp_color)
		AXIS.plot(comp_data['E'],comp_data['sig_scat']+comp_data['sig_abs'], lw=2,color=comp_color)
 	
	width = emax - emin
	xtext = emin + 0.05*width

	AXIS.text(xtext,1.1*sig_abs_func(xtext), "abs", fontsize=10,va='top')
	AXIS.text(xtext,1.1*sig_scat_func(xtext), "scat", fontsize=10,va='bottom')
	AXIS.text(xtext,1.1*sig_abs_func(xtext) + sig_scat_func(xtext), "ext",fontsize=10,va='bottom')

	AXIS.text(xlab,ylab,name,va='top',ha='left', transform=AXIS.transAxes, fontsize=20)
	for line in lines:
		AXIS.axvline(line,ls=':',color='k')
		AXIS.text(line+1,1.05*ymax,"%.1f"%(line),va='bottom',ha='left',fontsize=10)
...
# Si K edge
set_axis(1810.0,1890.0,"Si K",ax11,lines=[1845.6],comp_data=comparison_data)
...
# Fe K edge
set_axis(7050.0,7150.0,"Fe K",ax12,lines=[7123.0],comp_data=comparison_data)
ax12.xaxis.set_major_locator(plt.MultipleLocator(20))
ax12.xaxis.set_minor_locator(plt.MultipleLocator(5))
# Fe L edge
set_axis(695.0,735.0,"Fe L",ax21,lines=[707.8,720.6],comp_data=comparison_data)
ax21.xaxis.set_major_locator(plt.MultipleLocator(20))
ax21.xaxis.set_minor_locator(plt.MultipleLocator(5))
# Mg K edge
set_axis(1295.0,1335.0,"Mg K",ax22,lines=[1310.6],comp_data=comparison_data)
...
'''
# O K edge
set_axis(515.0,555.0,"O K",ax32,lines=[537.6],comp_data=comparison_data)
